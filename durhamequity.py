...
def genBlockGroupData(var):
    return censusdata.download('acs5', year, censusdata.censusgeo([
        ('state', nc), ('county', durhamCounty), ('block group', '*')]), var)
...
durham = genBlockGroupData(all_var)

# compute variables
durham['income'] = (durham.B19001_002E + durham.B19001_003E + 
        durham.B19001_004E + durham.B19001_005E) / (durham.B19001_001E) * 100
durham['rent'] = (durham.B25070_007E + durham.B25070_008E + durham.B25070_009E + 
        durham.B25070_010E) / (durham.B25070_001E - durham.B25070_011E) * 100
durham['minority'] = (durham.B03002_001E - durham.B03002_003E) / (durham.B03002_001E) * 100
durham['older'] = (durham.B11007_002E) / (durham.B11007_001E) * 100
durham['younger'] = (durham.B11005_002E) / (durham.B11005_001E) * 100
durham['femaleHouseholder'] = (durham.B17010_016E + durham.B17010_036E) / (durham.B17010_001E) * 100

durham = durham[['income', 'rent', 'minority', 'older', 'younger', 'femaleHouseholder']]

maxVal = []
percent = ['income', 'rent', 'minority', 'older', 'younger', 'femaleHouseholder']
for i in range(len(percent)): 
    maxVal += [durham.describe()[percent[i]]['max']]

score = durham.copy().div(maxVal) # here
...
equityIndexFiveYear = score.sort_values('mean', ascending=False).head(10)
# equityIndexFiveYear.to_csv('1year_sample.csv')


sample = censusdata.download('acs5', 2015,
...
# change formula for normalization
# find block group names
# difference between 1 and 5 years
